...
def get_shipments(status_filter="all", search="", date_from=None, date_to=None):
    query = supabase.table("plat_terminal_shipments").select("*")
...
def get_shipment(shipment_id):
    response = (
        supabase.table("plat_terminal_shipments")
        .select("*")
        .eq("id", shipment_id)
        .maybe_single()
        .execute()
...
def get_history(shipment_id):
    response = (
        supabase.table("plat_terminal_shipments_status_history")
        .select("*")
...
def create_shipment(venue_id, supplier_id, address, contact_name, notes, created_by):
    ...
    supabase.table("plat_terminal_shipments_status_history").insert(
        {
...
def update_shipment(shipment_id, updates, changed_by="", status_note=""):
    ...
    if changes:
        ...
        supabase.table("plat_terminal_shipments").update(changes).eq("id", shipment_id).execute()
...
def delete_shipment(shipment_id):
    supabase.table("plat_terminal_shipments_status_history").delete().eq("shipment_id", shipment_id).execute()
    supabase.table("plat_terminal_shipments").delete().eq("id", shipment_id).execute()
